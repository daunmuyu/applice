...
function run() {
    ...
    var content = fs.readFileSync("./lang_zh_CN.ts").toString('utf8');
    var index = content.indexOf("items = {") + "items = {".length - 1;
    var lastindex = content.lastIndexOf("}");
    content = content.substr(index, lastindex - index);
    var zhCNDict;
    eval("zhCNDict=" + content);

    for (var p in zhCNDict) {
        zhCNDict[p] = traditionalized(zhCNDict[p]);
    }

    var str = `
export class lang_zh_TW {
    static langName = "zh-TW";
    static langDesc = "繁体中文";
    static items = {0};
}
`;

    str = str.replace("{0}", JSON.stringify(zhCNDict));

    fs.writeFileSync("./lang_zh_TW.ts", str, { encoding: "utf8" });

    console.log("lang_zh_TW.ts completed");

    var dir = "D:\\git codes\\bitcoinwin\\";
    var files = ["AMLKYCPolicies.zh_CN.html", "fengxian.zh_CN.html", "licai_fengxian.zh_CN.html", "PrivacyPolicy.zh_CN.html", "RegistrationAgreement.zh_CN.html", "TermsandConditions.zh_CN.html"
        , "rule\\zh_CN.htm", "aboutus\\index.zh_TW.html"];
    for (var i = 0; i < files.length; i++) {
        var filepath = dir + files[i];
        content = traditionalized(fs.readFileSync(filepath).toString('utf8'));

        filepath = dir + files[i].replace("zh_CN", "zh_TW");
        fs.writeFileSync(filepath, content, { encoding: "utf8" });
        console.log(filepath);
    }

    files = fs.readdirSync(dir + "Questions");

    for (var i = 0; i < files.length; i++) {
        var filepath = files[i];
        if (filepath.indexOf(".zh_CN.") > 0) {
            filepath = dir + "Questions\\" + files[i];
            content = traditionalized(fs.readFileSync(filepath).toString('utf8'));

            filepath = dir + "Questions\\" + files[i].replace("zh_CN", "zh_TW");
            fs.writeFileSync(filepath, content, { encoding: "utf8" });
            console.log(filepath);
        }
