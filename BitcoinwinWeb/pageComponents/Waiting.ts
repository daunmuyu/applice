...
export function registerWaiting() {
    Vue.component("waiting", {
        template: Component.requireHtml(html, <any>{ constructor: { name: "Waiting" } }),
        props: {
            /**如：审核中 */
            title: String,
            /**几秒后返回到什么 */
        ...
        methods: {
            ...
            countSeconds: function () {
                this.seconds = Math.max(0 , this._startSeconds - parseInt(<any>((new Date().getTime() - this._startTime) / 1000)));
                if (this.seconds == 0) {
                    this.$emit('back');
                }
                else {
