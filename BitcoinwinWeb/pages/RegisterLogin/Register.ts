...
export class Register extends BaseComponent {
        ...
        setTimeout(() => {
            AccountCenterApi.GetCountryInfoList(this, (ret, err) => {
                ...
                if (err)
                    ...
                    ret.forEach((item) => {
                        this.model.countries.push(item);
                    });
                    this.model.selectedCountry = "86";
                }
            });
        }, 100);
    }
    ...
    sendsms() {
        ...
        AccountCenterApi.SendSmsCode(this, this.model.username, parseInt(this.model.selectedCountry), 1, (ret, err) => {
            this.model.isBusy = false;
            if (err)
                showError(err);
            else {
   ...
   async submit() {
       ...
       var result = ModelValidator.verifyToProperty(this.model, arr, "validator");
        ...
        if (!this.model.agree) {
            await  alertWindow(this.model.textRes.items["请阅读并同意注册协议"]);
            return;
        }
        this.model.isBusy = true;
        ...
        AccountCenterApi.CheckPhoneSmsCode(this, this.model.username, this.model.verifyCode, 1, (ret, err) => {
            if (err) {
                this.model.isBusy = false;
                showError(err);
            }
            else {
                AccountCenterApi.Register(this, this.model.username, this.model.pwd1, this.model.selectedCountry,async (ret, err) => {
                    this.model.isBusy = false;
                    if (err)
                        showError(err);
                    else {
                        await   alertWindow(this.model.textRes.items["注册成功"]);
                        navigation.pop();
                    }
                });
            }
